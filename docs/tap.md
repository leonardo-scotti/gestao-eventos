# TERMO DE ABERTURA DE PROJETO - UniEvent
### HISTÓRICO DE VERSÕES
| Data | Autores | Descrição | Versões |
| --- | --- | --- | --- |
| 18/11/2025 | Leonardo Scotti([leonardo-scotti](https://github.com/leonardo-scotti)) | Criação do Documento | 1.0 |

## 1. Resumo Executivo
- Definição do projeto (visão, objetivos, escopo e entregáveis);
- Organização do projeto (usuários, stakeholders, papéis, responsabilidades);
- Planejamento do projeto (abordagem, cronograma, planejamento de recursos, financeiro e de qualidade);
- Considerações do projeto (riscos, suposições e restrições);

## 2. Definição do Projeto
### 2.1 Visão
Desenvolver uma aplicação web para criação, organização e gerenciamento de 
eventos.

O  sistema  permitirá  que  organizadores  cadastrem  eventos  com  detalhes 
completos  e  que  os  participantes  possam  visualizar,  se  inscrever  e  acompanhar  suas 
inscrições. Pode ser usado para eventos corporativos, culturais, educacionais ou sociais.

### 2.2 Objetivos
**Objetivos de negócio:**
- Sistema funcional desenvolvido por completo;
- Agilizar processos de organização dos eventos (divulgação, venda de ingressos, controle de vagas, gestão de palestrantes e comunicação 
com  o  público.).
- Agilizar processo de reserva de vagas por parte dos clientes;
- Facilitar o generiamento dos eventos por parte da organização;

**Objetivos de tecnologia:**
- Desenvolver um Sistema com foco na Gerenciamento de Eventos.

### 2.3 Escopo
- Agilidade no processo de reserva de vaga em um evento, por parte do usuário;
- Desenvolvimento de Sistema Web para gerenciamento de Eventos, tais como processos de: divulgação, venda de ingressos, controle de vagas, gestão de palestrantes e comunicação 
com  o  público;

### 2.4 Entregáveis
| Item | Componentes | Descrição |
| ---| --- | --- | 
| Documentação | 1. Protótipo<br> 2. Diagramas de Classe<br> 3. Documentação da API<br> 4. Levantamento de Requisitos| 1. WireFrame;<br> 2. Alta fidelidade;<br> 3. Modelo Conceitual e Lógico;<br> 4. Swagger;<br> 5. Requisitos Funcionais (RF) e Não Funcionais (RNF); |
| Código | 1. Primeira Versão | 1. Entrega das funcionalidades confome foi pedido. |

## 3. Organização do Projeto
### 3.1 Usuários
| Usuário |
| --- |
| 1. Organizadores de Eventos;<br> 2. Participantes que buscam se inscrever e acompanhar eventos;<br> 3. Secretarias acadêmicas, departamentos de RH, organizadores de WorkShops e palestras.

### 3.2 Stakeholders
| Stakholders | Interesse|
| --- | --- |
| [Leonardo Scotti](https://github.com/leonardo-scotti) | Desenvolvedor do Projeto |
| [Vitor Miguel](https://github.com/Vfy-gif) | Desenvolvedor do Projeto |
| [João Vitor](https://github.com/Joaoblesa) | Desenvolvedor do Projeto |
| [Fernando Leonid](https://github.com/fernandoleonid) | Interessado na entrega do projeto |
| [Yuri Komuta](https://github.com/yurikomuta) | Interessado na entrega do projeto |
| [Celso Furtado](https://github.com/celsofurtado) | Interessado na entrega do projeto |
| [Marcel Teixeira](https://github.com/marcelInt) | Interessado na entrega do projeto |

### 3.3 Papéis
| Nome | Posições |
| --- | --- |
| [Leonardo Scotti](https://github.com/leonardo-scotti) | Desenvolvedor Front-end |
| [Vitor Miguel](https://github.com/Vfy-gif) | Desenvolvedor Back-end|
| [João Vitor](https://github.com/Joaoblesa) | DBA |

### 3.4 Responsabilidades
**Desenvolvedor Front-end:**
- Prototipagem;
- Código;
- Coerência - documentação e código;

**Desenvolvedor Back-end:**
- Documentação;
- Código;
- Coerência - documentação e código;

**DBA:**
- Modelo Conceitual;
- Modelo Lógico;
- Views;
- Triggers;

## 4. Planejamento do Projeto
### 4.1 Abordagem
| Fase | Abordagem |
|--- | --- |
| Iniciação | Autorização para o início do projeto. |
| Ideação | Definições do projeto, design e prototipação. |
| Criação | Planejamento, desenvolvimento teste e revisão. Processo irá se repetir em loop. |

### 4.2 Planejamento de Recursos
| Posição | Data de início | Data Final | Esforço |
| --- | --- | --- | --- |
| Front-end | 18/11/2025 | 18/12/2025 | 100% |
| Back-end | 18/11/2025 | 18/12/2025 | 100% |
| DBA | 18/11/2025 | 18/12/2025 | 100% |

## 5. Considerações do Projeto
### 5.1 Riscos
| Descrição | Probabilidade | Impacto | Ações de mitigação |
| :-------- | :-----------: | :-----: | :----------------- |
| Desistência de membro | Média | Alto | Acúmulo de função por membro que tiver com menos trabalho para fazer |
| Ausência constante de membro | Alta | Médio | Identificar o membro com problemas e entregar tarefas mais simples e menos importantes para ele |
| Atrasos por dificuldades com as tecnologias | Média | Baixo | Atraso inicial em entregas do software por dificuldades com as tecnologias utilizadas |
| Problemas com computadores de membro da equipe | Média | Baixo | Procurar outro computador para membro com problemas continuar o trabalho |
| Falta de comunicação | Alta | Médio | Motivar a equipe a se comunicar melhor, encorajar a comunicação, deixar claro a importância da comunicação e reforçar os meios de comunicação oficiais da equipe |

### 5.2 Suposições
- Esopo não irá mudar durante o Desenvolvimento do Software.

### 5.3 Restrições
- A quantidade de pessoas participanets do projeto não será aumentada até o final do projeto